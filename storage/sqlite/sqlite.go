...
func (s *Storage) PickRandom(ctx context.Context, Username string) (*storage.Page, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("can not pick random page: %w", err)
	}
...
func (s *Storage) Remove(ctx context.Context, p *storage.Page) error {
	...
	if _, err := s.db.ExecContext(ctx, q, p.URL, p.UserName); err != nil {
		return fmt.Errorf("can not remove page: %w", err)
	}
...
func (s *Storage) IsExists(ctx context.Context, p *storage.Page) (bool, error) {
	...
	if err := s.db.QueryRowContext(ctx, q, p.URL, p.UserName).Scan(count); err != nil {
		return false, fmt.Errorf("can not check if page exists: %w", err)
	}
...
func (s *Storage) Init(ctx context.Context) error {
	...
	if _, err := s.db.ExecContext(ctx, q); err != nil {
		return fmt.Errorf("can not create table: %w", err)
	}
